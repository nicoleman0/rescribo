...
Set these values in the shell or an ignored local file:
...
export RESCRIBO_GITHUB_REDIRECT_URI=https://your-callback.example/callback
```

Start a ten-minute, single-use authorisation session. Use a synthetic workspace identifier and bind the expected disposable repository into the state:

```sh
task github-installation-check -- start \
  --workspace feasibility-a \
  --repository owner/disposable-repository
```

Open the printed URL. After authorising the GitHub App, copy the `code` and `state` values from the callback URL into the completion command:

```sh
task github-installation-check -- complete \
  --workspace feasibility-a \
  --state ... \
  --code ...
```
